    ...
    try:
        ...
        for i in agg_list:
            if i.availability_zone is not None:
                agg_info.append(i)
        for i in agg_info:
            if re.match(stro + agg + stro, i.name.split('@')[1],
                         flags=re.IGNORECASE):
                if re.match(stro + zone + stro, i.availability_zone,
                            flags=re.IGNORECASE):
...
def getResource(client, nodes):
    ...
    hyper_info = client.hypervisors.list(False)
    node_info = []
...
def printData(data):
    ...
    print_data['1.Name'] = data.get('name').split('@')[1]
