...
from cinderclient.v1 import client as cclient1
...
class NovaAction():
    ...
    def cinderClient(self, obj):
        try:
            conn = cclient.Client(username=obj.username, api_key=obj.passwd,
                                  project_id=obj.name, auth_url=obj.url, service_type='volume',
                                  retries=5, timeout=1000, http_log_debug=True)
    ...
    def cinderClientV1(self, obj):
        try:
            conn = cclient1.Client(username=obj.username, api_key=obj.passwd,
                                  project_id=obj.name, auth_url=obj.url, service_type='volume',
                                  retries=5, timeout=1000, http_log_debug=True
                                  )
        except Exception, e:
    ...
    def keystoneClient(self, obj):
        try:
            conn = kclient.Client(username=obj.username,
                                  password=obj.passwd,
    ...
    def suspendInstances(self, vm_id, client):
            try:
